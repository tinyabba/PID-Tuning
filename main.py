...
actions = np.zeros((n_trials, horizon, p))
obj_fun = np.zeros((n_trials, horizon))
done_flag = False
...
#Compute optimal PID parameters for each trial
...
for K in pid_actions:
    ...
    i += 1
for trial_i in range(n_trials):
    K_opt[trial_i] = pid_actions[np.argmin(all_SSE[:, trial_i])]
...
actions = runner.perform_simulations()
