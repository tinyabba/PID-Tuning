...
#Define noises
np.random.seed(1)
noise_temp = np.random.normal(0, sigma, (n_trials, horizon, n))
out_noise_temp = np.random.normal(0, sigma, (n_trials, horizon, m))
noise = np.zeros((n_trials+1, horizon, n))
out_noise = np.zeros((n_trials+1, horizon, m))
noise[0:9] = noise_temp
out_noise[0:9] = out_noise_temp
...
env = PIDTuningEnvironment(A, b, c, n, p, m, y_0, horizon, noise[9:,:,:], out_noise[9:,:,:], 2)
